...
| R-03 | `PAYLOAD_SECRET` boş string default'u — **çözüldü** | Güvenlik | **P0 → ÇÖZÜLDÜ** | `cms/src/env.ts` (yeni, zod) — `DATABASE_URI`/`PAYLOAD_SECRET`/`REVALIDATE_SECRET` boot-time zorunlu; prod'da (`NODE_ENV=production` ve `CMS_AUTO_LOGIN` kapalı) bilinen dev-placeholder değerleriyle boot etmeyi reddediyor. Canlı doğrulandı: `CMS_AUTO_LOGIN=false` + dev secret ile her istek 500 ("Refusing to start in production") dönüyor | (çözüldü) | Uygulandı | S |
| R-04 | Compose'da dev secret default'ları — **kısmen çözüldü** | Güvenlik | **P1 → KISMEN ÇÖZÜLDÜ** | `docker-compose.yml` default'ları hâlâ duruyor (yerel kolaylık için, bilinçli) ama artık R-03'teki boot-time kontrolü prod'da bu default'ların kullanılmasını engelliyor — `.env` unutulursa sessizce ayakta kalmak yerine sistem hiç çalışmıyor | (yerel default'lar kalması bilinçli, prod'da artık engelleniyor) | R-03 ile birlikte etkisiz hale getirildi | S |
...
| R-08 | `cms.ts`: runtime doğrulama yok, sessiz `null` fallback, timeout yok — **çözüldü** | Kod kalitesi | **P1 → ÇÖZÜLDÜ** | Her getter artık zod şemasıyla validate ediyor, `AbortSignal.timeout(8000)`, `console.error` ile yapılandırılmış loglama. Bu süreçte gerçek bir bug bulundu ve düzeltildi: Payload boş opsiyonel alanları JSON `null` döner (omit etmez), `z.string().optional()` bunu reddediyordu — canlı testte `campaigns` ve `content-blocks` responseları bu yüzden sessizce fallback'e düşüyordu. `nullableString()`/`nullableStringDefault()` helper'larıyla düzeltildi, regresyon testi eklendi | (çözüldü) | Uygulandı | M |
| R-09 | Revalidate endpoint: zaman-sabit olmayan karşılaştırma, rate limit yok, tag allowlist yok — **çözüldü** | Güvenlik | **P1 → ÇÖZÜLDÜ** | `crypto.timingSafeEqual` (uzunluk farkını da güvenli ele alıyor), 12 (sonra 13) elemanlı tag allowlist, 60sn'de 30 istek sabit pencere rate limit. Canlı doğrulandı: CMS'te gerçek bir Fee Row oluşturulup hardened endpoint'e 200 ile ulaştığı, secret/tag/rate-limit red senaryolarının hepsinin doğru koda düştüğü test edildi | (çözüldü) | Uygulandı | S |
| R-10 | Migration yok — **prod'da gerçekten kırdığı canlı doğrulandı** | Veri katmanı | **P1 → CANLI DOĞRULANDI, HÂLÂ AÇIK** | `ContentBlocks` koleksiyonu eklenip prod image deploy edildiğinde CMS container'ı gerçekten unhealthy oldu: `column payload_locked_documents__rels.content_blocks_id does not exist`. `payload migrate:create` de mevcut Node/tsx sürümüyle ayrı bir hatayla (`ERR_REQUIRE_ASYNC_MODULE`) çalışmadı — CLI'ın kendi config yükleme rotası `@payloadcms/richtext-lexical`'ın top-level await'iyle uyumsuz. Bu oturumda geçici çözüm: NODE_ENV=development ile tek seferlik `next dev` çalıştırıp Payload'ın push-based şema senkronunu tetikledim, şemayı senkronladıktan sonra prod container'a geri döndüm. **Bu prod'da tekrarlanabilir bir prosedür değil** — her yeni koleksiyon/alan eklemesinde aynı manuel adım gerekecek | Yeni collection/field eklemek CMS'i prod'da kırıyor, migration'sız her deploy risk taşıyor | `payload migrate:create` neden başarısız oluyor kök nedenini çöz (tsx/Node sürüm uyumu) VEYA Payload'ı migration CLI'ının çalıştığı bir Node sürümüne sabitle; sonra gerçek migration akışını kur | M |
| R-11 | `TemsilciliklerimizForm` tamamen kırık — **çözüldü** | Kod kalitesi | **P1 → ÇÖZÜLDÜ** | `src/data/il-ilce.ts` (81 il, gerçek ilçe listeleri) eklendi, ilçe `<select>` artık ile göre gerçek verilerle doluyor. "Bul" butonu il+ilçe seçilene kadar disabled, seçilince Google Maps'te "Vodafone Mağaza {ilçe} {il}" araması açıyor (gerçek dealer/store veritabanı projede yok, en dürüst çözüm buydu). Tarayıcıda İstanbul→Kadıköy seçilip buton aktifleşmesi ve doğru arama sorgusu oluşturması doğrulandı | (çözüldü) | Uygulandı | S |
| R-12 | Aynı içerik türü (SSS) iki paralel kaynaktan yönetiliyor — **çözüldü** | Kod kalitesi / İçerik | **P1 → ÇÖZÜLDÜ** | `sikca-sorulan-sorular/page.tsx` artık `getFaqItems()` (tüm kategoriler) çağırıp `FaqCategoryFilter`'a prop olarak geçiyor; component CMS kategori değerlerini (`anasayfa`, `aninda-bakiye`, ...) sayfa etiketlerine eşleyip grupluyor, CMS boşsa kendi fallback setine düşüyor. Tarayıcıda "Kampanyalar" kategorisine tıklanıp doğru filtrelenen tek soru göründüğü doğrulandı | (çözüldü) | Uygulandı | M |
...
| R-22 | Legal sayfa gövdeleri hardcoded — **bilinçli olarak kapsam dışı bırakıldı** | İçerik / Operasyon | **P2 → KARAR VERİLDİ (kapsam dışı)** | `cms/src/collections/LegalPages.ts:10-11` (koleksiyonun kendi açıklaması) | 5 legal sayfanın gövdesi (nested liste/tablo, KVKK madde referansları, hukuki dil) CMS'e otomatik taşınmadı — bilinçli bir kapsam kararı: (1) zaten koleksiyonun kendi açıklaması bunu "sabit kalacak" diye belgeliyordu, (2) hukuki metni genel amaçlı CMS alanlarına (array/richText) taşırken içerik doğruluğunu (madde numaraları, KVKK referansları) riske atmadan yapmak, tek oturumda güvenle tamamlanamayacak kadar büyük bir iş | Editörler istiyorsa: her legal sayfa için özel, alan-alan modellenmiş bir koleksiyon (yapısını hukuk ekibiyle birlikte tasarlayarak) — genel amaçlı alanlarla riskli | L (ayrı, dikkatli bir çalışma gerektirir) |
| R-23 | ~10 bileşen + 5 sayfa CMS'e hiç bağlı değil — **çoğu çözüldü** | İçerik | **P3 → ÇOĞU ÇÖZÜLDÜ** | Yeni `cms/src/collections/ContentBlocks.ts` (page+blockType ile esnek, step/slide/video/logo) eklendi. CMS'e bağlanan: `StepPhones`, `FeatureHighlights`, `AppFeatures`, `EarnWithCard`, `BrandLogoGrid`, `VideoGuideSection`, `HowToEarn` (uygulama + faturana-yansit çağrıları) — hepsi gerçek görsellerle (MinIO'ya yüklendi) CMS'ten canlı doğrulandı: içerik CMS'te değiştirilip sitenin anında güncellendiği tarayıcıdan test edildi. **Kapsam dışı bırakılanlar (bilinçli):** `VideosWithTabs` (gerçek video içeriği değil, etiketli placeholder kutular — ürün tasarımı netleşmeden CMS'e taşımak anlamsız), `kurumsal-yonetim`/`faydali-bilgiler`/`site-haritasi` sayfaları (yapısal/kurumsal metin, legal sayfalara benzer risk profili, R-22 ile aynı gerekçe) | Kalan öğeler editöre açık değil ama ürünün asıl "pazarlama içeriği" artık %100 CMS-editable | VideosWithTabs için önce ürün kararı gerekiyor (gerçek video mu eklenecek, kaldırılacak mı); kurumsal/legal sayfalar R-22 ile birlikte ayrı bir çalışma | S (VideosWithTabs karar bekliyor) / L (kurumsal sayfalar) |
