...
**Follow-up 28.08.2026 — Growth'un permission ŞEKLİ değişti (yukarıdaki "4 permission şekli hâlâ sabit" artık 3 gerçek şekil):** Kullanıcı gerçek AccessPoint rol matrisini paylaştı; 4 satırından 3'ü ("biz"/geliştirici satırı hariç) kabul edildi. `roleMapping.ts`'teki eşleme zaten bu tam AccessPoint ID'leriyle yazılı olduğu için değişmedi — değişen, Growth Maker/Checker'ın "sadece Campaigns" olan kapsamının New Vertical ile birebir aynı içerik kapsamına genişlemesiydi (`cms/src/access/roles.ts`'teki yeni `standardCreate`/`standardReadWrite`/`standardDelete`/`denyMakerEditPublished`, `cms/src/lib/collectionLabels.ts`'teki `GROWTH_DASHBOARD_COLLECTIONS`). Detay: `tasks.md` §28, `scripts/growth-role-migration-28-08.sql`.
