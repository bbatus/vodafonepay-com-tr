// Auto-generated by `npm run generate:importmap` (wraps `payload
// generate:importmap`). Previously this file had to be hand-maintained
// because the CLI crashed with ERR_REQUIRE_ASYNC_MODULE — see
// docs/STATUS.md R-10 for the root cause (cms/package.json was missing
// `"type": "module"`, so tsx transpiled payload.config.ts to CommonJS and
// `require()`'d `@payloadcms/richtext-lexical`, an ESM package with
// top-level await) and the fix (added `"type": "module"`, plus
// `admin.importMap.baseDir` in payload.config.ts so the generator resolves
// the `/components/...` shorthand against `src/`, not the project root).
// Re-run `npm run generate:importmap` after adding/removing a custom admin
// component or a `richText` editor feature — do not hand-edit this file.
